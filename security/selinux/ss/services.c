		       ...
		       int *len, char ***names, int **values)
{
	...
	*values = kcalloc(*len, sizeof(int), GFP_ATOMIC);
	...
	for (i = 0; i < *len; i++) {
		...
		rc = -ENOMEM;
		(*names)[i] = kstrdup(policydb->p_bools.p_key_val_to_name[i],
				      GFP_ATOMIC);
		if (!(*names)[i])
			goto err;
	}
...
static int _security_set_bools(struct selinux_state *state,
			       int len, int *values)
{
	struct policydb *policydb = &state->ss->policydb;
	int i, rc = 0;
	...
	for (i = 0; i < len; i++)
		policydb->bool_val_to_struct[i]->state = values[i];
...
int security_set_bools(struct selinux_state *state,
		       int len, int *values)
{
	return _security_set_bools(state, len, values);
}
				   ...
				   struct policydb *policydb)
{
	...
	rc = security_get_bools(state, &nbools, &names, &values);
	if (rc)
		return rc;
	...
	for (i = 0; i < nbools; i++) {
		...
		booldatum = hashtab_search(policydb->p_bools.table, names[i]);
		if (booldatum)
			booldatum->state = values[i];
	}
			    ...
			    int bool)
{
	...
	if (bool < 0 || bool >= policydb->p_bools.nprim) {
		rc = -EINVAL;
		goto out;
	}
